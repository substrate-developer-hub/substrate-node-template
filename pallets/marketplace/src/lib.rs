...
pub mod pallet {
	...
	pub(super) type Sellers<T: Config> = StorageMap< _,Blake2_128Concat, T::AccountId,Vec<u8>,ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn get_iso_country)]
	pub(super) type IsoCountries<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,Vec<u8>,ValueQuery>;
	
	#[pallet::storage]
	#[pallet::getter(fn get_products_department)]
	pub(super) type ProductDepartments <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	
	#[pallet::storage]
	#[pallet::getter(fn get_products_category)]
	pub(super) type ProductCategories <T: Config> = StorageDoubleMap< _,Blake2_128Concat, u32,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;

    #[pallet::storage]
	#[pallet::getter(fn get_currency)]
	pub(super) type Currencies<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,Vec<u8>,ValueQuery>;    

	#[pallet::storage]
	#[pallet::getter(fn get_products_color)]
	pub(super) type ProductColors <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	
	#[pallet::storage]
	#[pallet::getter(fn get_products_size)]
	pub(super) type ProductSizes <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	...
	pub(super) type ShippingRates <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	
	#[pallet::storage]
	#[pallet::getter(fn get_iso_dialcode)]
	pub(super) type IsoDialcode<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,Vec<u8>,ValueQuery>;    

	#[pallet::storage]
	#[pallet::getter(fn get_login_data)]
	pub(super) type LoginData<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,Vec<u8>,ValueQuery>;    

	#[pallet::storage]
	#[pallet::getter(fn get_email_account)]
	pub(super) type EmailAccount<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,T::AccountId>;    
	
	#[pallet::storage]
	#[pallet::getter(fn get_encrypted_seed)]
	pub(super) type EmailEncryptedSeed<T: Config> = StorageMap< _,Blake2_128Concat, Vec<u8>,Vec<u8>,ValueQuery>;    
	
	#[pallet::storage]
	#[pallet::getter(fn get_product_model)]
	pub(super) type ProductModels <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	...
	pub(super) type Manufacturers <T: Config> = StorageMap< _,Blake2_128Concat, u32,Vec<u8>,ValueQuery>;
	...
	impl<T: Config > Pallet<T> {
        ...
        pub fn create_iso_country(origin: OriginFor<T>, countrycode: Vec<u8>, countryname: Vec<u8>) -> DispatchResult {
            ...
            ensure!(!IsoCountries::<T>::contains_key(&countrycode), Error::<T>::CountryCodeAlreadyPresent);
		 ...
		 pub fn destroy_iso_country(origin: OriginFor<T>, countrycode: Vec<u8>,) -> DispatchResult {
			 ...
			 ensure!(IsoCountries::<T>::contains_key(&countrycode)==true, Error::<T>::CountryCodeNotFound);
			 // Remove country code
		 ...
		 #[pallet::call_index(4)]
		 #[pallet::weight(T::WeightInfo::cause_error())]
		 pub fn create_product_department(origin:OriginFor<T>, uid: u32, description: Vec<u8>) -> DispatchResult {
			 ...
			 ensure!(ProductDepartments::<T>::contains_key(uid)==false, Error::<T>::DepartmentAlreadyPresent);
		 ...
		 pub fn destroy_product_department(origin:OriginFor<T>, uid: u32) -> DispatchResult {
			 ...
			 ensure!(ProductDepartments::<T>::contains_key(&uid)==true, Error::<T>::DepartmentNotFound);
			 // Remove department
		 ...
		 #[pallet::call_index(6)]
		 #[pallet::weight(T::WeightInfo::cause_error())]
		 pub fn create_product_category(origin:OriginFor<T>, uiddepartment: u32, uidcategory: u32, description: Vec<u8>) -> DispatchResult {
			 // check the request is signed from the Super User
			 let _sender = ensure_root(origin)?;
			 ...
			 ensure!(ProductDepartments::<T>::contains_key(uiddepartment)==true, Error::<T>::DepartmentNotFound);
			 // check the department/category is not alreay present on chain
			 ensure!(ProductCategories::<T>::contains_key(uiddepartment,uidcategory)==false, Error::<T>::ProductCategoryAlreadyPresent);
			 // store the department
			 ProductCategories::<T>::insert(uiddepartment,uidcategory,description.clone());
			 // Generate event
			 Self::deposit_event(Event::MarketPlaceCategoryCreated(uiddepartment,uidcategory,description));
			 // Return a successful DispatchResult
			 Ok(())
		 }
		 ...
		 #[pallet::call_index(7)]
		 #[pallet::weight(T::WeightInfo::cause_error())]
		 pub fn destroy_product_category(origin :OriginFor<T>, uiddepartment: u32, uidcategory: u32) -> DispatchResult {
			 // check the request is signed from the Super User
			 let _sender = ensure_root(origin)?;
			 // verify the department/category exists
			 ensure!(ProductCategories::<T>::contains_key(&uiddepartment,&uidcategory)==true, Error::<T>::ProductCategoryNotFound);
			 // Remove department
		  ...
		  #[pallet::call_index(8)]
		  #[pallet::weight(T::WeightInfo::cause_error())]
		  pub fn create_update_seller(origin:OriginFor<T>, configuration: Vec<u8>) -> DispatchResult {
			  ...
			  loop {  
				  ...
				  ensure!(ProductCategories::<T>::contains_key(categoryu32,departmentu32)==true, Error::<T>::ProductCategoryNotFound);
				  x=x+1;
				  nc=nc+1;
			  }
			  ...
			  loop {  
				  ...
				  ensure!(IsoCountries::<T>::contains_key(country)==true, Error::<T>::CountryCodeNotFound);
		...
		#[pallet::weight(T::WeightInfo::cause_error())]
        pub fn create_update_product(origin:OriginFor<T>, uid: u32, configuration: Vec<u8>) -> DispatchResult {
            // check the request is signed
            ...
            loop {
                ...
                if color.len()>0 {
                    ...
                    ensure!(ProductColors::<T>::contains_key(&colorvalue), Error::<T>::ColorNotFound);
                }
                x=x+1;
            }
		...
		pub fn create_dialcode_country(origin:OriginFor<T>, countrycode: Vec<u8>, dialcode: Vec<u8>) -> DispatchResult {
			// check the request is signed from the Super User
			let _sender = ensure_root(origin)?;
			// check country code length == 2
			ensure!(countrycode.len()==2, Error::<T>::WrongLengthCountryCode);
			...
			ensure!(IsoDialcode::<T>::contains_key(&countrycode)==false, Error::<T>::CountryCodeAlreadyPresent);
		...
		#[pallet::weight(T::WeightInfo::cause_error())]
		pub fn destroy_dialcode_country(origin:OriginFor<T>, countrycode: Vec<u8>,) -> DispatchResult {
			// check the request is signed from the Super User
			let _sender = ensure_root(origin)?;
			// verify the country code exists
			ensure!(IsoDialcode::<T>::contains_key(&countrycode)==true, Error::<T>::CountryCodeNotFound);
			// Remove country code
		...
		/// Create a new Currency code with name and other info in a json structure
		 ...
		 #[pallet::weight(T::WeightInfo::cause_error())]
		 pub fn create_currency(origin:OriginFor<T>, currencycode: Vec<u8>, info: Vec<u8>) -> DispatchResult {
			 // check the request is signed from the Super User
			 let _sender = ensure_root(origin)?;
			 ...
			 ensure!(!Currencies::<T>::contains_key(&currencycode), Error::<T>::CurrencyCodeAlreadyPresent);
		 ...
		 #[pallet::weight(T::WeightInfo::cause_error())]
		 pub fn create_product_color(origin:OriginFor<T>, uid: u32, description: Vec<u8>) -> DispatchResult {
			 // check the request is signed from root
			 let _sender = ensure_root(origin)?;
			 // check uid >0
			 ...
			 ensure!(ProductColors::<T>::contains_key(uid)==false, Error::<T>::ColorAlreadyPresent);
			 // store the color
		 ...
		 pub fn destroy_product_color(origin:OriginFor<T>, uid: u32) -> DispatchResult {
			 ...
			 ensure!(ProductColors::<T>::contains_key(&uid)==true, Error::<T>::ColorNotFound);
			 // Remove color
		...
		#[pallet::weight(T::WeightInfo::cause_error())]
        pub fn create_product_size(origin:OriginFor<T>, uid: u32, info: Vec<u8>) -> DispatchResult {
            // check the request is signed from root
            let _sender = ensure_root(origin)?;
            // check uid >0
        ...
        pub fn destroy_product_size(origin:OriginFor<T>, uid: u32) -> DispatchResult {
            ...
            ensure!(ProductSizes::<T>::contains_key(&uid)==true, Error::<T>::SizeNotFound);
            // Remove size
		...
		#[pallet::weight(T::WeightInfo::cause_error())]
        pub fn create_manufacturer(origin:OriginFor<T>, uid: u32, info: Vec<u8>,) -> DispatchResult {
            // check the request is signed from root
            let _sender = ensure_root(origin)?;
            // check uid >0
        ...
        pub fn create_shipper(origin:OriginFor<T>, uid: u32, info: Vec<u8>,) -> DispatchResult {
            ...
            if origincountries.len()>0 {
                ...
                loop {  
                    ...
                    ensure!(IsoCountries::<T>::contains_key(oc),Error::<T>::OriginCountryNotPresent);
                    x=x+1;
                }
            }
            ...
            if destinationcountries.len()>0 {
                ...
                loop {  
                    ...
                    ensure!(IsoCountries::<T>::contains_key(dc),Error::<T>::DestinationCountryNotPresent);
                    x=x+1;
                }
            }
            // store the shippers
        ...
        pub fn destroy_shipper(origin:OriginFor<T>, uid: u32) -> DispatchResult {
		  ...
		  #[pallet::weight(T::WeightInfo::cause_error())]
          pub fn create_shipping_rates(origin:OriginFor<T>, uid: u32, info: Vec<u8>,) -> DispatchResult {
            // check the request is signed from root
            let _sender = ensure_root(origin)?;
            // check uid >0
            ...
            ensure!(IsoCountries::<T>::contains_key(origincountry.clone()),Error::<T>::OriginCountryNotPresent);
            ...
            if rates.len()>0 {
                ...
                loop {  
                    ...
                    ensure!(IsoCountries::<T>::contains_key(dc),Error::<T>::OriginCountryNotPresent);
                    // check for fromkg
        ...
        pub fn destroy_brand(origin:OriginFor<T>, uid: u32) -> DispatchResult {
		 ...
		 #[pallet::weight(T::WeightInfo::cause_error())]
         pub fn create_product_model(origin:OriginFor<T>, uid: u32, info: Vec<u8>,) -> DispatchResult {
             // check the request is signed from root
             let _sender = ensure_root(origin)?;
             // check uid >0
		 ...
		 #[pallet::weight(T::WeightInfo::cause_error())]
        pub fn create_login_data(origin:OriginFor<T>, emailhash: Vec<u8>, encryptedpwdhash: Vec<u8>, accountid: T::AccountId,encryptedseed: Vec<u8>) -> DispatchResult {
            // check the request is signed 
            let _sender = ensure_signed(origin)?;
            ...
            ensure!(LoginData::<T>::contains_key(&emailhash)==false, Error::<T>::EmailHashAlreadyPresent);
            ...
            // Generate event
            Self::deposit_event(Event::MarketPlaceLoginDataCreated(emailhash,encryptedpwdhash,accountid));
            // Return a successful DispatchResult
            Ok(())
        }
        /// Create a new Login Data
		...
		#[pallet::weight(T::WeightInfo::cause_error())]
        pub fn change_pwd_login_data(origin:OriginFor<T>, emailhash: Vec<u8>, encryptedpwdhash: Vec<u8>) -> DispatchResult {
        ...
        pub fn destroy_login_data(origin:OriginFor<T>, emailhash: Vec<u8>,) -> DispatchResult {
            ...
            ensure!(LoginData::<T>::contains_key(&emailhash)==true, Error::<T>::EmailHashNotFound);
            ...
            ensure!(accountid==core::prelude::v1::Some(sender),Error::<T>::SignerIsNotAuthorized);
